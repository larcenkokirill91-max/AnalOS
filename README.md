...
* Для любителей графики (C/C++): У нас готов вывод пикселей через GOP. Нужно написать простую библиотеку для отрисовки оконных виджетов (кнопки, текстовые поля) поверх нашего фреймбуфера. А если вы ещё хорошо разбираетесь в том как можно связать C и C++ код то вы нам очень нужны.
