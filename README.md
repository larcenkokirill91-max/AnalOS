...
- [ ] 1.1. Создание MBR-загрузчика на NASM (512 байт, сигнатура 0xAA55).
- [ ] 1.2. Настройка вывода символов через прерывание BIOS INT 10h (функция 0x0E).
- [ ] 1.3. Написание скрипта автоматизации сборки (Makefile) и запуск бинарного образа в QEMU.
- [ ] 1.4. Перенос загрузчика на физическую USB-флешку и успешный тест на реальном ПК.

- [ ] 2.1. Описание таблицы дескрипторов сегментов (GDT) для кода, данных и стека.
- [ ] 2.2. Установка бита PE в регистре CR0 и выполнение дальнего прыжка (Far Jump) для очистки конвейера процессора.
- [ ] 2.3. Открытие адресной линии A20 через контроллер клавиатуры (порт 0x64) или Fast A20 (порт 0x92).
- [ ] 2.4. Прямой вывод символов в текстовую видеопамять VGA (запись байт по адресу 0x000B8000).

- [ ] 3.1. Конфигурация GCC/Clang для сборки "freestanding" (без стандартной библиотеки, флагов -ffreestanding, -nostdlib).
- [ ] 3.2. Написание скрипта линкера (linker.ld) для выравнивания секций ядра (.text, .data, .bss) в памяти.
- [ ] 3.3. Создание таблицы векторов прерываний (IDT) и регистрация обработчиков исключений процессора (ISR 0-31).
- [ ] 3.4. Перепрограммирование контроллера прерываний PIC 8259 (сдвиг базовых векторов IRQ 0-15 на диапазон 0x20-0x2F).
- [ ] 3.5. Реализация обработки прерывания таймера (IRQ 0) и клавиатуры (IRQ 1, чтение скан-кодов из порта 0x60).
...
- [ ] 6.1. Написание UEFI-загрузчика на C/Rust (вместо устаревшего BIOS) с использованием протокола GOP (Graphics Output Protocol).
- [ ] 6.2. Переключение процессора в 64-битный режим (Long Mode) с пятиуровневым пейджингом (Paging).
- [ ] 6.3. Перепрограммирование контроллеров прерываний на современные стандарты APIC и IOAPIC.

- [ ] 7.1. Создание линейного кадрового буфера (Linear Framebuffer) для отрисовки пикселей в высоком разрешении.
- [ ] 7.2. Разработка шрифта фиксированной ширины (парсинг .psf матриц) для вывода текста в графическом режиме.
- [ ] 7.3. Инициализация PS/2 или USB контроллера мыши, перехват прерывания IRQ 12.
- [ ] 7.4. Создание логики курсора: сохранение пикселей под мышью в буфер, отрисовка стрелки, восстановление фона при движении.
