...
Для знатоков Rust/C++ (Алгоритмы): Нужен чистый драйвер файловой системы FAT16/32. Придётся немного повозится с железом.
Для любителей графики (C++): У нас готов вывод пикселей через GOP. Нужно написать простую библиотеку для отрисовки оконных виджетов (кнопки, текстовые поля) поверх нашего фреймбуфера. А если вы ещё хорошо разбираетесь в том как можно связать C и C++ код то вы нам очень нужны.
