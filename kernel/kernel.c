...
// 1. Идеально чистая привязка буфера к отметке 64 МБ
...
// 2. Выносим pitch_dw в глобальные переменные, чтобы draw_time её видела!
int pitch_dw = 1280; 
...
// 3. Твоя сверхбыстрая функция копирования
        ...
        if (status & 0x01) {
            ...
            if (status & 0x20) {
                ...
                if (mouse_cycle == 3) {
                    ...
                    if (left_button == 1) {
                        // МОМЕНТ ЗАХВАТА: Кнопка только что нажата, окно еще не тащат
                        if (prev_left_button == 0 && dragged_window_idx == -1 && start_menu_open == 0) {
                            for (int i = 0; i < 3; i++) {
                                if (win[i].is_visible && mouse_x >= win[i].lt.x && mouse_x <= (win[i].lt.x + win[i].width) && mouse_y >= win[i].lt.y && mouse_y <= (win[i].lt.y + WIN_HH)) {
                                    ...
                                    // Запоминаем Самые Начальные Координаты Окна!
        ...
        if (current_scancode != 0) {
            ...
            if (menu_open == 1) {
                if (data == 28) {
                    ...
                    selected_option = 1;
                    draw_rect((unsigned char*)back_buffer32, 500, 565, 100, 4, 255, 255, 255, 255);
                    draw_rect((unsigned char*)back_buffer32, 750, 565, 100, 4, 0, 120, 212, 255);
                    swap_buffers();
                    draw_cursor((unsigned char*)video_memory32, mouse_x, mouse_y);
                } 
                else if (data == 75) {
                    selected_option = 0;
                    draw_rect((unsigned char*)back_buffer32, 750, 565, 100, 4, 255, 255, 255, 255);
                    draw_rect((unsigned char*)back_buffer32, 500, 565, 100, 4, 0, 120, 212, 255);
                    swap_buffers();
                    draw_cursor((unsigned char*)video_memory32, mouse_x, mouse_y);
                }
